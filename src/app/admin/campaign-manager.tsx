...
import type { AdminReservationDTO } from "@/modules/reservations/service";
...
export function CampaignManager({ initialCampaigns, dashboard, initialReservations }: { initialCampaigns: CampaignDTO[]; dashboard: DashboardDTO; initialReservations: AdminReservationDTO[] }) {
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="rounded-3xl border border-white/10 bg-white/5 p-6 sm:p-8"><div className="flex items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-widest text-emerald-300">Participantes</p><h2 className="mt-1 text-2xl font-black">Reservas mais recentes</h2></div><span className="text-sm text-slate-500">{initialReservations.length} registros</span></div>{initialReservations.length === 0 ? <p className="mt-6 text-slate-400">Nenhum número escolhido ainda.</p> : <div className="mt-6 overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="border-b border-white/10 text-xs uppercase tracking-wider text-slate-500"><tr><th className="px-3 py-3">Data</th><th className="px-3 py-3">Nome</th><th className="px-3 py-3">Contato</th><th className="px-3 py-3">Campanha</th><th className="px-3 py-3">Números</th></tr></thead><tbody className="divide-y divide-white/5">{initialReservations.map((reservation, index) => <tr key={`${reservation.campaignId}-${reservation.createdAt}-${index}`} className="hover:bg-white/[0.03]"><td className="whitespace-nowrap px-3 py-4 text-slate-400">{reservation.createdAt ? dateTime.format(new Date(reservation.createdAt)) : "—"}</td><td className="px-3 py-4 font-bold text-white">{reservation.participantName}</td><td className="px-3 py-4 text-slate-300">{reservation.contact}</td><td className="px-3 py-4 text-slate-300">{reservation.campaignTitle}</td><td className="px-3 py-4"><div className="flex flex-wrap gap-2">{reservation.numbers.map((number) => <span key={number} className="inline-flex min-w-8 justify-center rounded-full bg-red-500/20 px-2.5 py-1 font-black text-red-200">{number}</span>)}</div></td></tr>)}</tbody></table></div>}</section>
    </div>
  );
}
