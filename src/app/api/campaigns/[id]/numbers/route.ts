...
import { listReservedNumbers, reserveNumbers } from "@/modules/reservations/service";
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json({ reservations: await reserveNumbers(id, await request.json()) }, { status: 201 });
  } catch (error) {
    if (error instanceof ZodError) return NextResponse.json({ error: "Confira o nome, telefone e números escolhidos.", fields: error.flatten().fieldErrors }, { status: 400 });
    const code = error instanceof Error ? error.message : "UNKNOWN";
    const status = code === "NOT_FOUND" ? 404 : code === "NUMBER_UNAVAILABLE" ? 409 : code === "CAMPAIGN_NOT_OPEN" ? 409 : 400;
    const message = code === "NUMBER_UNAVAILABLE" ? "Um dos números acabou de ser reservado. Atualize e escolha novamente." : code === "CAMPAIGN_NOT_OPEN" ? "Esta campanha não está recebendo reservas." : code === "NUMBER_OUT_OF_RANGE" ? "Um número está fora do intervalo da campanha." : "Não foi possível reservar os números.";
